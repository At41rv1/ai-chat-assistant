...
const JWT_SECRET = '528927342667872763787278625518900888776362';
...
app.post('/api/auth/google', async (req, res) => { /* ... Paste from previous version ... */ });
app.post('/api/auth/signup', async (req, res) => { /* ... Paste from previous version ... */ });
app.post('/api/auth/login', (req, res) => { /* ... Paste from previous version ... */ });
...
app.get('/api/admin/chats/:userId', verifyToken, isAdmin, (req, res) => {
    ...
    db.all('SELECT role, content, timestamp FROM messages WHERE user_id = ? ORDER BY timestamp ASC', [userId], (err, rows) => {
        if (err) return res.status(500).json({ message: "Error fetching chat history." });
        res.json(rows);
    });
});
...
// Fill in the auth routes from the previous version
app.post('/api/auth/google', async (req, res) => {
    ...
    } catch (error) { res.status(400).json({ message: 'Invalid Google token' }); }
});
app.post('/api/auth/signup', async (req, res) => {
    ...
    db.run('INSERT INTO users (id, username, password_hash, role) VALUES (?, ?, ?, ?)', [id, username, password_hash, 'user'], function (err) {
        if (err) return err.message.includes('UNIQUE') ? res.status(409).json({ message: 'Username already taken.' }) : res.status(500).json({ message: 'Database error.' });
...
app.post('/api/auth/login', (req, res) => {
    ...
    db.get('SELECT * FROM users WHERE username = ?', [username], async (err, user) => {
        if (err || !user || !user.password_hash) return res.status(401).json({ message: 'Invalid credentials.' });
